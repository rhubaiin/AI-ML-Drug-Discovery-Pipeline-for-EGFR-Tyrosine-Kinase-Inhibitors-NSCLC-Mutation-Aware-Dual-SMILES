#!/usr/bin/env python3
"""
Prediction script for adv_physchem KAN B-Spline model
Hierarchical 6-site RNN Model with KANLayer (B-Spline basis)

This script loads trained hierarchical site-specific models (with KANLayer)
...
# ============================================================================
# FEATURE GENERATION FUNCTIONS (Copied from adv_physchem KAN B-Spline)
# ============================================================================
...
# ============================================================================
# KANLayer (B-Spline) - Required for loading model 2 weights
# ============================================================================
...
class KANLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.mu = tf.cast(grid, dtype=tf.float32)
    ...
    def B_batch(self, x, knots):
        ...
        batch_flat = tf.reshape(x, (-1, orig_shape[-1]))

        t_i = knots[:-1]
        t_ip1 = knots[1:]
        x_exp = tf.expand_dims(batch_flat, -1)
        t0 = tf.reshape(t_i, (1, 1, -1))
        t1 = tf.reshape(t_ip1, (1, 1, -1))
        N = tf.cast(tf.logical_and(x_exp >= t0, x_exp < t1), tf.float32)
    ...
    def call(self, x):
        ...
        B = self.B_batch(x, extended_knots)
...
print("=" * 80)
print("PREDICTION SCRIPT FOR ADV_PHYSCHEM KAN B-SPLINE")
print("Hierarchical 6-site RNN Model with KANLayer (B-Spline basis)")
print("=" * 80)
...
def load_models_and_scalers(model_dir):
    """Load all required models and scalers (with KANLayer custom objects)"""
    ...
    for site_name in MUTATION_SITES:
        ...
        if os.path.exists(model_path):
            hierarchical_models[site_name] = load_model(model_path, compile=False, custom_objects=custom_objects)
    ...
    if os.path.exists(rnn_path):
        rnn_model = load_model(rnn_path, compile=False, custom_objects=custom_objects)
...
def evaluate_and_plot(df_results, output_dir, model_name):
    ...
    # Overall combined plot (2x2 layout)
    ...
    for mutation in mutations:
        ...
        axes[0].set_title(f'Activity\nPearson R = {mut_metrics["Activity_Pearson_R"]:.4f}, p = {mut_metrics["Activity_Pearson_pval"]:.4e}', fontsize=11)
        axes[0].legend()
        ...
        axes[1].set_title(f'Docking\nPearson R = {mut_metrics["Docking_Pearson_R"]:.4f}, p = {mut_metrics["Docking_Pearson_pval"]:.4e}', fontsize=11)
        axes[1].legend()
...
def make_predictions(input_csv, model_dir='.', output_dir='.'):
    """
    Make predictions using the trained hierarchical RNN model (with KANLayer).
    ...
    # Pre-create embedding models once
    embedding_models = {}
    for site_name in MUTATION_SITES:
    ...
    output_path = os.path.join(output_dir, 'predictions_kan_bspline.csv')
    ...
    if has_ground_truth and len(df_results) > 0:
        evaluate_and_plot(df_results, output_dir, 'kan_bspline')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Predictions for adv_physchem KAN B-Spline model')
