...
print("="*80)
print("RNN-LSTM INTEGRATED HIERARCHICAL MODEL")
print("Sequential Training: ATP_POCKET -> P_LOOP -> C_HELIX -> DFG_A_LOOP -> HRD_CAT")
print("="*80)

# === Load Data ===
print("\nLoading datasets...")
df_train = pd.read_csv('/mnt/d/Publications/project_insilico/activity_physiochem/df_combo_tki_ed_smiles_lig_activity_validated.csv')
df_control = pd.read_csv('/mnt/d/Publications/project_insilico/activity_physiochem/egfr_tki_valid_cleaned.csv')
df_drugs = pd.read_csv('/mnt/d/Publications/project_insilico/activity_physiochem/drugs.csv')
...
print("✓ Saved mutation_profiles.csv")
...
#     # 2. Calculate Sine Metric
#     # Method A: Sine of the angle (Derived from Cosine)
#     # Formula: sin(theta) = sqrt(1 - cos(theta)^2)
#     # This represents the magnitude of the orthogonal component.
...
def generate_hierarchical_features(ligand_smiles_series, mutation_smiles_series):
    ...
    for idx, (lig_smi, mut_smi) in enumerate(zip(ligand_smiles_series, mutation_smiles_series)):
        ...
        # generating descriptor features for intermolecular and intramolecular ligand and mutation
...
def build_priority_hierarchical_model(feature_dims):
    """
    Build hierarchical model with priority-based feature processing.
    ...
    logger.info(f"  Element-wise Multiply: (batch, 48) ⊙ (batch, 48) -> (batch, 48)")
    ...
    logger.info(f"  Gated Multiply: (batch, 32) ⊙ (batch, 32) -> (batch, 32)")
    ...
    # Final prediction
    output = Dense(1, activation='linear', kernel_initializer='glorot_uniform', name='activity_output')(embedding_output)
    logger.info(f"\n[Final Output]")
    logger.info(f"  Dense(1, linear): (batch, 16) -> (batch, 1)")
    logger.info(f"  Predicts binding affinity (continuous value)")
...
# t=2: P_LOOP_HINGE    → ATP binding region
# t=3: C_HELIX         → Regulatory region
# t=4: DFG_A_LOOP      → Activation loop
# t=5: HRD_CAT         → Catalytic site

# # Backward LSTM/GRU reads in reverse:
# t=5: HRD_CAT         → Catalytic outcome
# t=4: DFG_A_LOOP      → Activation state
# t=3: C_HELIX         → Regulatory input
# t=2: P_LOOP_HINGE    → Binding mechanics
# t=1: ATP_POCKET      → Active site
# t=0: FULL_SMILES     → Overall context

#RNN Model Receives Temporal Sequence
...
def build_rnn_sequential_model(embedding_dim, n_timesteps=6):
    """
    Build RNN-LSTM model to process sequential mutation embeddings.
    Logs the shape transformations through recurrent layers.
    
    Args:
        embedding_dim: Dimension of each timestep embedding (16-dim from hierarchical model)
        n_timesteps: Number of sequential mutations (5 mutation sites)
    ...
    logger.info(f"    Forward LSTM: reads t=0,1,2,3,4,5 → output (batch, {n_timesteps}, 128)")
    logger.info(f"    Backward LSTM: reads t=5,4,3,2,1,0 → output (batch, {n_timesteps}, 128)")
    ...
    logger.info(f"    Forward GRU: reads t=0,1,2,3,4 → (batch, {n_timesteps}, 128)")
    logger.info(f"    Backward GRU: reads t=4,3,2,1,0 → (batch, {n_timesteps}, 128)")
    logger.info(f"  Concatenate: → (batch, {n_timesteps}, 256)")
    logger.info(f"    where z_t is update gate, h̃_t is candidate activation")
    logger.info(f"  Output: (batch, {n_timesteps}, 256)")
    ...
    # Final output
    output = Dense(1, activation='linear', name='final_activity_output')(x)
    logger.info("\n" + "="*80)
    logger.info("FINAL OUTPUT")
    logger.info("="*80)
    logger.info(f"  Dense(1, linear): (batch, 32) → (batch, 1)")
    logger.info(f"  Linear activation: No transformation, direct regression output")
    logger.info(f"  Output interpretation: Predicted binding affinity for entire mutation sequence")
    
    model = Model(inputs=sequence_input, outputs=output, name='rnn_sequential_model')
...
def main():
    ...
    # Get y_train
    y_train = activity_values_valid.iloc[common_valid_indices].values
    y_train = np.log1p(y_train)  # use log1p (log(1+x)) to avoid errors with 0 values

    y_scaler = StandardScaler()
    y_train_scaled = y_scaler.fit_transform(y_train.reshape(-1, 1)).flatten()
    
    with open('activity_scaler.pkl', 'wb') as f:
        pickle.dump(y_scaler, f)
    print("✓ Saved activity_scaler.pkl")
    ...
    # Save feature scalers after loop
    ...
    for profile_idx, profile_row in unique_mutation_profiles.iterrows():
        ...
        for site_idx, (site_name, _) in enumerate(mutation_sites):
            ...
            hierarchical_model = load_model(f'hierarchical_model_{site_name}.h5')
        ...
        if len(control_embeddings_all_sites) == 6 and control_valid_idx is not None:
            ...
            control_predictions = rnn_model.predict(control_sequential, verbose=0).flatten()
            control_predictions = y_scaler.inverse_transform(control_predictions.reshape(-1, 1)).flatten()
            control_predictions = np.expm1(control_predictions)  # Inverse of log1p

            
            for idx, pred in zip(control_valid_idx, control_predictions):
            ...
            print(f"    Control: min={control_predictions.min():.4f}, max={control_predictions.max():.4f}, mean={control_predictions.mean():.4f}")
        ...
        if len(drug_embeddings_all_sites) == 6 and drug_valid_idx is not None:
            ...
            drug_predictions = rnn_model.predict(drug_sequential, verbose=0).flatten()
            drug_predictions = y_scaler.inverse_transform(drug_predictions.reshape(-1, 1)).flatten()
            drug_predictions = np.expm1(drug_predictions)  # Inverse of log1p
            
            for idx, pred in zip(drug_valid_idx, drug_predictions):
            ...
            print(f"    Drug: min={drug_predictions.min():.4f}, max={drug_predictions.max():.4f}, mean={drug_predictions.mean():.4f}")
    ...
    axes[1].plot(rnn_history.history['mae'], label='Train MAE', linewidth=2, color='#2E86AB')
    axes[1].plot(rnn_history.history['val_mae'], label='Val MAE', linewidth=2, color='#A23B72')
    ...
    print("="*80)
    print("✓ 5 Hierarchical models: hierarchical_model_*.h5")
