#!/usr/bin/env python3
"""
Prediction script for adv_physchem5e
...
# Import common feature generation functions
...
print("=" * 80)
print("PREDICTION SCRIPT FOR ADV_PHYSCHEM5E")
print("Hierarchical 6-site RNN Model")
print("=" * 80)

MUTATION_SITES = ['FULL_SMILES', 'ATP_POCKET', 'P_LOOP', 'C_HELIX', 'DFG_A_LOOP', 'HRD_CAT']
...
def load_models_and_scalers(model_dir):
    ...
    # Load numeric scalers (feature_scalers.pkl)
    ...
    if os.path.exists(scalers_path):
        ...
        raise FileNotFoundError(f"Feature scalers not found: {scalers_path}")

    # Load activity scaler (activity_scaler.pkl)
    activity_scaler_path = os.path.join(model_dir, 'activity_scaler.pkl')
    if os.path.exists(activity_scaler_path):
        with open(activity_scaler_path, 'rb') as f:
            y_scaler1 = pickle.load(f)
        print(f"  ✓ Loaded activity scaler")
    else:
        # Fallback if not found (though training script should save it)
        print(f"  Warning: Activity scaler not found, using default StandardScaler")
        from sklearn.preprocessing import StandardScaler
        y_scaler1 = StandardScaler()
    
    return hierarchical_models, rnn_model, all_scalers, y_scaler1
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Predictions for adv_physchem5e')
