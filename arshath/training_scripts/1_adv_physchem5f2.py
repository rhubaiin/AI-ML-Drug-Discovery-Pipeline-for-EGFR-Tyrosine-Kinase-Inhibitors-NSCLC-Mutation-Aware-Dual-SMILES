...
def main(output_dir='.', train_data=None, control_data=None, drug_data=None):
    ...
    for site_idx, (site_name, _) in enumerate(mutation_sites):
        ...
        checkpoint = ModelCheckpoint(
            os.path.join(output_dir, f'hierarchical_model_{site_name}.h5'),
            monitor='val_loss',
            save_best_only=True,
            verbose=1
        )
    ...
    rnn_checkpoint = ModelCheckpoint(
        os.path.join(output_dir, 'rnn_sequential_model.h5'),
        monitor='val_loss',
        save_best_only=True,
        verbose=1
    )
    ...
    for site_name, _ in mutation_sites:
        h_model = load_model(os.path.join(output_dir, f'hierarchical_model_{site_name}.h5'), compile=False)
        h_model.compile(
            optimizer=Adam(learning_rate=0.003),
            loss={
                'activity_output': 'mean_squared_error',
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'rnn_training_history.png'), dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Advanced Physicochemical 5-Fold Hierarchical Model')
