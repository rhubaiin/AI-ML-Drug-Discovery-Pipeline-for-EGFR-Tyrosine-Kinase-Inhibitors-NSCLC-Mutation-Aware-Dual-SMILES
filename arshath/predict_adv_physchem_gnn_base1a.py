...
def make_predictions(input_csv, model_dir='.', output_dir='.'):
    ...
    # Use the corrected MUTATION_SITES that match training script
    # These must match the names used when saving models in training
    MUTATION_SITES_LOCAL = ['FULL_SMILES', 'ATP_POCKET', 'P_LOOP_HINGE', 'C_HELIX', 'DFG_A_LOOP', 'HRD_CAT']
    ...
    try:
        ...
        hierarchical_models = {}
        for site_name in MUTATION_SITES_LOCAL:
            model_path = os.path.join(model_dir, f'gnn_hierarchical_{site_name}.h5')
            if os.path.exists(model_path):
                hierarchical_models[site_name] = load_model(model_path, compile=False)
                print(f"  ✓ Loaded {site_name} model")
    ...
    for pred_idx, pred_row in df_pred.iterrows():
        ...
        for site_idx, (site_name, mutation_smiles) in enumerate(zip(MUTATION_SITES_LOCAL, mutation_smiles_list)):
