# Cross-Dataset Prediction Analysis (v2 - Re-run Feb 18, 2026)
...
All predictions were regenerated from scratch on Feb 18, 2026 using the same trained model weights.
...
|-------|:---:|:---:|:---:|:---:|
| 0 - Dummy PhysChem | 100,629,485 | 2,025,663,635 | 0.689 | 1.067 |
| 1 - Adv PhysChem 5F2 | 2,935 | 10,937 | 0.876 | 1.255 |
| 2 - KAN B-Spline | 2,566 | 11,103 | 0.867 | 1.248 |
| 3 - KAN Navier-Stokes | 2,891 | 10,839 | 0.692 | 1.057 |
| 4 - ChemBERTa CrossAttn | 2,714 | 11,115 | 0.682 | 1.052 |
| 5 - GNN/MolCLR | 2,563 | 11,128 | 1.049 | 1.415 |

### Direction B: manual trained --> predict on df_3_shuffled (n=748-750)

| Model | Activity MAE | Activity RMSE | Docking MAE | Docking RMSE |
|-------|:---:|:---:|:---:|:---:|
| 0 - Dummy PhysChem | 2,539 | 18,615 | 0.461 | 0.539 |
| 1 - Adv PhysChem 5F2 | 2,582 | 18,630 | 0.521 | 0.655 |
| 2 - KAN B-Spline | 2,547 | 18,646 | 2.240 | 2.430 |
| 3 - KAN Navier-Stokes | 2,529 | 18,608 | 0.488 | 0.584 |
| 4 - ChemBERTa CrossAttn | 2,549 | 18,648 | 0.460 | 0.602 |
| 5 - GNN/MolCLR | 2,547 | 18,641 | 0.630 | 0.739 |

---

## 5. Key Findings

### 5.1 Activity Predictions Do Not Transfer
...
- No model achieves meaningful activity generalization at the overall level
...
### 5.3 Per-Mutation Docking Transfer is Strong

Despite weak overall cross-prediction, per-mutation docking Pearson R values in Direction B are remarkably strong:
- `del`: R = 0.96 (M5), 0.76 (M3), 0.64 (M2)
- `del/t790m double`: R = 0.92 (M0), 0.89 (M4), 0.79 (M1)
- `ins 20`: R = 0.99 (M1), 0.98 (M4), 0.86 (M3)
- `l858r/t790m double`: R = 0.91 (M5), 0.83 (M4), 0.71 (M1)
- `l858r/t790m/c797s triple`: R = 0.92 (M5), 0.63 (M4)
- `subs l858r`: R = 0.96 (M0), 0.76 (M4), 0.69 (M1)
- `wild adeno lung`: R = 0.89 (M0), 0.79 (M4), 0.69 (M5)
...
### 5.4 Model 3 (KAN Navier-Stokes) Shows Best Activity Transfer
...
### 5.5 Model 5 (GNN) Has Asymmetric Transfer

Despite strong same-dataset performance (R=0.77 activity on manual), Model 5 GNN shows near-zero overall cross-prediction for activity. However:
- **Direction B docking** is strong (R=0.51 overall), driven by exceptional per-mutation correlations: del (0.96), l858r/t790m double (0.91), l858r/t790m/c797s triple (0.92), del/t790m/c797s triple (0.90)
- **Direction A** is poor across the board (Activity R=0.02, Docking R=0.02)

This asymmetry confirms that the GNN learns more transferable representations when trained on the larger dataset, but cannot generalize from the smaller one.

### 5.6 Model 0 (Dummy PhysChem) Best Docking Transfer

The simplest model (feedforward on physicochemical descriptors) shows the most consistent docking transfer:
- Direction A: R = 0.460
- Direction B: R = 0.490

Physicochemical descriptors are inherently more transferable than learned representations. Model 0 also has notably strong per-mutation docking R in Direction B: del/t790m double (0.92), subs l858r (0.96), wild adeno lung (0.89).

### 5.7 Model 2 (KAN B-Spline) Fails to Generalize

Model 2 shows near-zero or negative cross-dataset performance in both directions:
- Direction A: Activity R = 0.067, Docking R = 0.067
- Direction B: Activity R = -0.029, Docking R = 0.065 (not significant, p=0.076)

The B-spline KAN architecture consistently underperforms across all evaluation settings.
...
| Rank | Model | Direction A (R) | Direction B (R) | Average |
|:---:|-------|:---:|:---:|:---:|
| 1 | 3 - KAN Navier-Stokes | 0.170 | 0.019 | **0.095** |
| 2 | 1 - Adv PhysChem 5F2 | 0.073 | -0.024 | **0.025** |
| 3 | 2 - KAN B-Spline | 0.067 | -0.029 | **0.019** |
| 4 | 0 - Dummy PhysChem | 0.033 | 0.012 | **0.023** |
| 5 | 5 - GNN/MolCLR | 0.023 | -0.024 | **0.000** |
| 6 | 4 - ChemBERTa CrossAttn | -0.107 | 0.003 | **-0.052** |
...
6. **Model 5 (GNN) excels at per-mutation docking transfer** when trained on the larger dataset (Direction B), with multiple mutations achieving R > 0.90. However, it fails completely in Direction A.

7. **Model 2 (KAN B-Spline) should be deprioritized** -- it fails to generalize in any direction for either target.

8. **For deployment**, if cross-dataset generalization matters, consider:
   ...
   - Using an ensemble of Model 0 (docking) + Model 3 (activity) for maximum generalizability
