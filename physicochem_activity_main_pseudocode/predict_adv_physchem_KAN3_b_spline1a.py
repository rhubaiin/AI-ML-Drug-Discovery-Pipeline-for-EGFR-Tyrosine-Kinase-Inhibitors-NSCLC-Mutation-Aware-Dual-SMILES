#!/usr/bin/env python3
"""
Prediction script for adv_physchem_KAN3_b_spline1a.py
KAN with B-spline activation functions

This script loads trained KAN hierarchical models with B-spline layers
...
# ============================================================================
# CUSTOM GRU CLASS FOR KERAS VERSION COMPATIBILITY
# ============================================================================

class CustomGRU(GRU):
    """
    Custom GRU class to handle deprecated 'time_major' parameter
    that exists in older Keras models but is not supported in newer versions.
    """
    ...
    def from_config(cls, config):
        # Remove time_major from config if it exists
...
print("=" * 80)
print("PREDICTION SCRIPT FOR KAN B-SPLINE MODEL")
print("Hierarchical 6-site RNN with B-spline KAN layers")
print("=" * 80)

# ============================================================================
# KAN LAYER DEFINITION (B-SPLINE)
# ============================================================================
...
def load_models_and_scalers(model_dir):
    ...
    # Load RNN model with custom GRU to handle deprecated time_major parameter
...
def make_predictions(input_csv, model_dir='.', output_dir='.'):
    """
    Make predictions using trained KAN hierarchical models.
    
    Parameters:
    -----------
    input_csv : str
        Path to input CSV with columns: 'smiles', 'tkd' (mutation name)
    ...
    if 'smiles' not in df_pred.columns or 'tkd' not in df_pred.columns:
        raise ValueError("Input CSV must contain 'smiles' and 'tkd' columns")
    ...
    # Load mutation profiles (CSV not pickle!)
    ...
    # Load all scalers (feature_scalers.pkl not all_scalers.pkl!)
    print("\nLoading feature scalers...")
    scalers_path = os.path.join(model_dir, 'feature_scalers.pkl')
    with open(scalers_path, 'rb') as f:
        all_scalers = pickle.load(f)
    print(f"  Loaded scalers for {len(all_scalers)} sites")
    
    # Load y scalers (single file with dict!)
    print("\nLoading target scalers...")
    y_scalers_path = os.path.join(model_dir, 'y_scalers.pkl')
    with open(y_scalers_path, 'rb') as f:
        y_scalers_dict = pickle.load(f)
    y_scaler1 = y_scalers_dict['y_scaler1']
    y_scaler2 = y_scalers_dict['y_scaler2']
    
    # Load hierarchical models (6 sites including FULL_SMILES)
    print("\nLoading hierarchical models...")
    hierarchical_models = {
        'ATP_POCKET': load_model(os.path.join(model_dir, 'hierarchical_model_ATP_POCKET.h5'), 
                                custom_objects={'KANLayer': KANLayer}, compile=False),
        'P_LOOP_HINGE': load_model(os.path.join(model_dir, 'hierarchical_model_P_LOOP_HINGE.h5'),
                                  custom_objects={'KANLayer': KANLayer}, compile=False),
        'C_HELIX': load_model(os.path.join(model_dir, 'hierarchical_model_C_HELIX.h5'),
                             custom_objects={'KANLayer': KANLayer}, compile=False),
        'DFG_A_LOOP': load_model(os.path.join(model_dir, 'hierarchical_model_DFG_A_LOOP.h5'),
                                custom_objects={'KANLayer': KANLayer}, compile=False),
        'HRD_CAT': load_model(os.path.join(model_dir, 'hierarchical_model_HRD_CAT.h5'),
                             custom_objects={'KANLayer': KANLayer}, compile=False),
        'FULL_SMILES': load_model(os.path.join(model_dir, 'hierarchical_model_FULL_SMILES.h5'),
                                 custom_objects={'KANLayer': KANLayer}, compile=False)
    }
    print(f"  Loaded {len(hierarchical_models)} hierarchical models")
    
    # Create embedding models ONCE (not in the loop!)
    print("\nCreating embedding models...")
    from tensorflow.keras.models import Model
    embedding_models = {}
    for site_name, h_model in hierarchical_models.items():
        embedding_models[site_name] = Model(
            inputs=h_model.inputs,
            outputs=h_model.get_layer('embedding_output').output
        )
    print(f"  Created {len(embedding_models)} embedding models")
    
    # Load RNN model - Monkey patch to handle time_major parameter
    print("\nLoading RNN model...")
    from tensorflow.keras.layers import GRU, LSTM
    
    # Save original from_config methods
    original_gru_from_config = GRU.from_config
    original_lstm_from_config = LSTM.from_config
    
    # Create patched versions that remove time_major
    @classmethod
    def patched_gru_from_config(cls, config):
        config.pop('time_major', None)
        return original_gru_from_config(config)
    ...
    GRU.from_config = patched_gru_from_config
    LSTM.from_config = patched_lstm_from_config
    
    try:
        # Load model with patches in place
        rnn_model = load_model(
        ...
        GRU.from_config = original_gru_from_config
        LSTM.from_config = original_lstm_from_config
    ...
    for pred_idx, pred_row in df_pred.iterrows():
        ...
        # Get mutation SMILES for all sites (in correct order matching training)
        ...
        # Process through each hierarchical site IN THE EXACT ORDER AS TRAINING
        ...
        for site_idx, (site_name, mutation_smiles) in enumerate([
            ...
            # Generate features for this site
            ...
            # Scale features using the correct scaler for this site
            ...
            # Use pre-created embedding model (NOT creating it in the loop!)
            embedding_model = embedding_models[site_name]
            
            # Predict with features in correct order
        ...
        # Stack embeddings for RNN (shape: batch, 6 timesteps, embedding_dim)
    ...
    output_path = os.path.join(output_dir, 'predictions_adv_physchem_KAN3_b_spline1a.csv')
    df_results.to_csv(output_path, index=False)
    print(f"\n✓ Predictions saved to: {output_path}")
    
    # Evaluation (if ground truth exists)
    if has_ground_truth and len(df_results) > 0:
        evaluate_and_plot(df_results, output_dir, 'adv_physchem_KAN3_b_spline1a')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Predictions for KAN B-spline model')
