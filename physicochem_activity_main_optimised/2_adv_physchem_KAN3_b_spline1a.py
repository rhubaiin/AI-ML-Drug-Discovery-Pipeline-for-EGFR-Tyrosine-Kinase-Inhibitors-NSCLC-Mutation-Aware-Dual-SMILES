...
# Data loading is performed inside main() for parameterization support
...
def main(output_dir='.', train_data=None, control_data=None, drug_data=None):
    ...
    print(f"Training history plot saved: {os.path.join(output_dir, 'kan_training_history.png')}")
    ...
    #         h_model = load_model(os.path.join(output_dir, f'hierarchical_model_{site_name}.h5'), custom_objects={'KANLayer': KANLayer}, compile=False)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='KAN B-Spline Hierarchical Model')
