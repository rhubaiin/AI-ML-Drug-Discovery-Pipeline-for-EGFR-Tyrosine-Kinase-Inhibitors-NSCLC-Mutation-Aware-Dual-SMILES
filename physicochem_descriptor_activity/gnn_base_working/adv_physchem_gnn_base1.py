...
def get_gnn_embeddings(smiles_list, model, device, batch_size=32):
    ...
    if TORCH_GEOMETRIC_AVAILABLE:
        ...
        # Fallback mode: use fingerprints
        for i in range(0, len(smiles_list), batch_size):
            batch_smiles = smiles_list[i:i+batch_size]
            fps = np.array([smiles_to_fingerprint(smi) for smi in batch_smiles])
            fps_tensor = torch.tensor(fps, dtype=torch.float32).to(device)
            
            with torch.no_grad():
                embeddings = model(fps_tensor).cpu().numpy()
...
def test_mode(n_samples=5):
    """
    Test mode: Run with limited samples to verify architecture works.
    """
    print("\n" + "="*80)
    print(f"TEST MODE: Running with {n_samples} samples")
    print("="*80)
    ...
    df_test = df_train.head(n_samples).copy()
    
    # Validate and filter
    valid_mask = ~(
        df_test['smiles'].isna() |
        df_test['smiles_full_egfr'].isna() |
        df_test['smiles 718_862_atp_pocket'].isna() |
        df_test['smiles_p_loop'].isna() |
        df_test['smiles_c_helix'].isna() |
        df_test['smiles_l858r_a_loop_dfg_motif'].isna() |
        df_test['smiles_catalytic_hrd_motif'].isna() |
        df_test['standard value'].isna() |
        df_test['dock'].isna()
    )
    ...
    print(f"Valid samples: {len(df_test)}")
    
    # Initialize GNN model (random weights for testing)
    device = torch.device('cpu')
    gnn_model = GINet(num_layer=5, emb_dim=300, feat_dim=512)
    gnn_model.to(device)
    gnn_model.eval()
    print("âœ“ GINet model initialized (random weights)")
    
    # Extract GNN embeddings for ligands
    lig_smiles_list = df_test['smiles'].astype(str).tolist()
    print(f"Extracting GNN embeddings for {len(lig_smiles_list)} ligands...")
    lig_gnn_embs = get_gnn_embeddings(lig_smiles_list, gnn_model, device)
    print(f"  Ligand GNN embeddings shape: {lig_gnn_embs.shape}")
    ...
    for site_name, site_col in mutation_sites:
        ...
        mut_smiles_series = df_test[site_col]
        
        # Generate hierarchical features
        feature_dict = generate_hierarchical_features(df_test['smiles'], mut_smiles_series)
        valid_idx = feature_dict['valid_indices']
        
        if len(valid_idx) == 0:
            print(f"  ERROR: No valid features for {site_name}")
            continue
        
        # Get GNN embeddings for mutation site
        mut_smiles_list = mut_smiles_series.astype(str).tolist()
        mut_gnn_embs = get_gnn_embeddings(mut_smiles_list, gnn_model, device)
        
        # Concatenate ligand and mutation GNN embeddings
        gnn_concat = np.concatenate([lig_gnn_embs, mut_gnn_embs], axis=1)
        gnn_concat = gnn_concat[valid_idx]  # Filter to valid indices
        ...
        print(f"  âœ“ Hierarchical model built: {model.count_params():,} params")
        
        # Prepare targets
        y1 = np.log1p(df_test['standard value'].iloc[valid_idx].values)
        y2 = df_test['dock'].iloc[valid_idx].values
        ...
        print(f"  Training {site_name} (2 epochs)...")
        model.fit(
            x=[gnn_scaled, scaled_features['final_fp_interaction'],
               scaled_features['lig_mut_mix_inter_intra'], scaled_features['inter_interaction'],
               scaled_features['intra_interaction'], scaled_features['mut_inter'],
               scaled_features['lig_inter'], scaled_features['mut_intra'],
               scaled_features['lig_intra']],
            y={'activity_output': y1_scaled, 'docking_output': y2_scaled},
            epochs=2, batch_size=2, verbose=0
        )
        print(f"  âœ“ Training complete for {site_name}")
        
        # Extract embeddings
        embedding_model = Model(inputs=model.inputs, outputs=model.get_layer('embedding_output').output)
        embs = embedding_model.predict([
            gnn_scaled, scaled_features['final_fp_interaction'],
            scaled_features['lig_mut_mix_inter_intra'], scaled_features['inter_interaction'],
            scaled_features['intra_interaction'], scaled_features['mut_inter'],
            scaled_features['lig_inter'], scaled_features['mut_intra'],
            scaled_features['lig_intra']
        ...
        print(f"  âœ“ Embeddings extracted: {embs.shape}")
    
    if len(all_embeddings) != 6:
        print(f"\nWARNING: Only {len(all_embeddings)} sites completed")
        return False
    
    # Build RNN model
    print("\n--- Building RNN-LSTM Model ---")
    sequential_embs = np.stack(all_embeddings, axis=1)
    print(f"Sequential embeddings shape: {sequential_embs.shape}")
    
    rnn_model = build_rnn_sequential_model(embedding_dim=16, n_timesteps=6)
    print(f"âœ“ RNN model built: {rnn_model.count_params():,} params")
    
    # Quick RNN training
    print("Training RNN (2 epochs)...")
    rnn_model.fit(
        x=sequential_embs,
        y={'final_activity_output': y1_scaled, 'final_docking_output': y2_scaled},
        epochs=2, batch_size=2, verbose=0
    )
    print("âœ“ RNN training complete")
    
    # Final prediction test
    preds = rnn_model.predict(sequential_embs, verbose=0)
    print(f"\nPredictions:")
    print(f"  Activity: {preds[0].flatten()}")
    print(f"  Docking: {preds[1].flatten()}")
    
    print("\n" + "="*80)
    print("TEST MODE COMPLETE - All components working!")
    print("="*80)
    return True
...
def keras_main():
    ...
    for i, (site_name, _) in enumerate(mutation_sites):
        row, col = i // 4, i % 4
        if i < len(all_histories):
            axes[row, col].plot(all_histories[i]['loss'], label='Train')
            axes[row, col].plot(all_histories[i]['val_loss'], label='Val')
            axes[row, col].set_title(site_name)
            axes[row, col].legend()
            axes[row, col].grid(True, alpha=0.3)
    
    axes[1, 2].plot(rnn_history.history['loss'], label='Train')
    axes[1, 2].plot(rnn_history.history['val_loss'], label='Val')
    axes[1, 2].set_title('RNN-LSTM')
    axes[1, 2].legend()
    axes[1, 2].grid(True, alpha=0.3)
    axes[1, 3].axis('off')
    
    plt.tight_layout()
    plt.savefig('gnn_training_history.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("[OK] Saved gnn_training_history.png")
