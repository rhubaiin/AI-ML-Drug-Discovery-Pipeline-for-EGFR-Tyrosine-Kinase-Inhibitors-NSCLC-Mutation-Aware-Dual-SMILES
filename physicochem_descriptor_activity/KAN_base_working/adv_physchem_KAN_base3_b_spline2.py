...
# ============================================================================
# B-SPLINE KAN LAYER (adapted from PyKAN spline.py)
# ============================================================================
...
class KANLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, out_features, grid_size=20, grid_range=[-1.0, 1.0], degree=3, **kwargs):
        ...
        self.grid_size = int(grid_size) # Number of grid intervals (G)
        self.grid_min, self.grid_max = float(grid_range[0]), float(grid_range[1]) # Domain [-1, 1]
        self.k = int(degree) # B-spline degree (k=3 for cubic)

    def build(self, input_shape): #Building weights
        ...
        coeff_count = max(1, self.grid_size + self.k)
    ...
    def extend_grid(self, grid):
        ...
        h = (grid[-1] - grid[0]) / tf.cast(self.grid_size - 1, tf.float32)
        
        extended = grid
        for i in range(self.k):
            left_point = extended[0:1] - h
            right_point = extended[-1:] + h
            extended = tf.concat([left_point, extended, right_point], axis=0)
    ...
    def B_batch(self, x, knots):
        ...
        final = tf.where(tf.math.is_nan(final), tf.zeros_like(final), final)
